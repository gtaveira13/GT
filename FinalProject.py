...
def loading_data(filepath):
    ...
    dfCrashes['Month'] = dfCrashes['CRASH_DATE'].dt.month  # [DA1]
    dfCrashes = dfCrashes.dropna(subset=['LAT', 'LON'])
    return dfCrashes


def lineplot(dfCrashes, show_grid=True):  # [PY1], [VIZ1]
    MonthlyCrashes = dfCrashes.groupby('Month').size().reset_index(name='Crashes')      # [DA2]
    selectedMonth = st.slider('Select a month', 1, 12, 1)   # [ST2]
    selectedMonthName = monthsList[selectedMonth - 1]
    selectedMonthCrashes = MonthlyCrashes[MonthlyCrashes['Month'] == selectedMonth]['Crashes'].iloc[0]  # [DA4]
    st.write(f"Total crashes in {selectedMonthName}: {selectedMonthCrashes}")

    plt.figure(figsize=(8, 6))
    plt.plot(MonthlyCrashes['Month'], MonthlyCrashes['Crashes'], marker='o')
...
def barplot(dfCrashes):    # [VIZ2]
    monthlyData = dfCrashes.groupby('Month').size().reset_index(name='Crashes')   # [DA7]
    fig, ax = plt.subplots(figsize=(10, 6))
    colors = plt.cm.plasma(np.linspace(0, 1, 12))    # [ST4]
...
def piechart(data, column, title):  # [VIZ3]
    count_data = data[column].value_counts()
    percent_data = (count_data / count_data.sum()) * 100
    other_threshold = 2.5
    small_categories = percent_data[percent_data < other_threshold].index
    count_data['Other'] = count_data.loc[small_categories].sum()
    count_data = count_data.drop(small_categories)
    fig, ax = plt.subplots()
    # Explode the smallest pie pieces to improve label visibility
    explode = [0.1 if value < other_threshold else 0 for value in count_data]  # [PY4]
    wedges, texts, autotexts = ax.pie(count_data, labels=count_data.index, autopct='%1.1f%%', startangle=90, explode=explode,  pctdistance=0.85)
    # Pull out the percentage labels to the inside a bit for better spacing
    plt.setp(autotexts, size=7, weight="bold")
    ax.axis('equal')
    # Adjust the legend's position to not overlap with slices
    plt.legend(title='Categories', loc='center left', bbox_to_anchor=(1.2, 0.5))
    plt.title(title)
    st.pyplot(fig)


def map_display(dfCrashes):  # [VIZ4]
    # Defining the layer for the map
    layer = pdk.Layer(
        ...
        get_color='[200, 30, 0, 160]',  # Color of markers
...
def crashStats(dfCrashes):  # [PY2]
    monthlyData = dfCrashes.groupby('Month').size().reset_index(name='Crashes')
    totalCrashes = monthlyData['Crashes'].sum()   # [DA9]
    maxCrashes = monthlyData['Crashes'].max()     # [DA3]
...
st.image("C:\\Users\\tavei\\OneDrive - Bentley University\\Spring 2024\\CS 230\\FictionalCarCrash.webp") # [VIZ4]   # Utilized GPT to generate image
...
view_options = st.selectbox('Choose the data view', ['Bar Plot', 'Pie Chart', 'Line Graph', 'Accident Map'])   # [ST1]
if view_options == 'Bar Plot':
    ...
    st.write('The distribution is relatively uniform but we can see that '
             ...
             ' the crash total were elevated compared to the Spring months.')   # [ST3]
